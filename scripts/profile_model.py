...
def main() -> None:
    ...
    parser.add_argument("--router-mode", default="prototype", choices=["prototype", "direct", "static", "uniform"])
    parser.add_argument("--batch-size", type=int, default=128)
    args = parser.parse_args()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    cfg = ModelConfig(num_channels=args.num_channels, num_classes=args.num_classes, router_mode=args.router_mode)
    model = SETMClassifier(cfg).to(device)
